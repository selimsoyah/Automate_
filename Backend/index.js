...
app.post("/addcar", (req, res) => {
  ...
  db.query(
    "INSERT INTO car(Brand,Type,Kilo,Age,Insurance,Emptying,Visit) VAlUES (?,?,?,?,?,?,?)",
    [Brand, Type, Kilo, Age, Insurance, Empty, Visit],
    (err, result) => {
      if (err) {
        console.log(err);
  ...
  db.query(
    ...
    (err, result) => {
      if (result.length > 0) {
        ...
        const token = jwt.sign({ id, mecorcar }, "jwtSecretKey", {
          expiresIn: 300,
        });
